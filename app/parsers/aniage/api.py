...
async def addon_stream(id: str, session: aiohttp.ClientSession = Depends(get_session)):
    streams = {"streams": []}
    ...
    async with session.get(f"{mainUrl}/watch?wid={id}") as response:
        ...
        for voice in meta_json["props"]["pageProps"]["teams"]:
            ...
            async with session.get(
                f"{apiUrl}/anime/episodes?animeId={id}&page=1&pageSize=30&sortOrder=ASC&teamId={voice['teamId']}&volume=1"
            ) as episodes:
                index = 0
